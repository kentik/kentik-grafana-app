...
export interface UDEQueryTarget extends DataQuery {
  ...
  cidr?: number;
  cidr6?: number;
  /** "table" reduces each series to one row (value = mean) for maps/tables;
   *  "nodegraph" builds nodes+edges from two dimensions for the Node Graph panel. */
  format?: 'table' | 'nodegraph';
}
