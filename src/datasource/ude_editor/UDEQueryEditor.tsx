...
import { Stack, Field, Combobox, ComboboxOption, Input, Button } from '@grafana/ui';
...
export const UDEQueryEditor: React.FC<Props> = ({ query, onChange, onRunQuery, datasource }) => {
  ...
  return (
    <Stack direction="column" gap={1}>
      ...
      {current.measurement && (
        <>
          <Field label="Units (Metrics)" invalid={Boolean(queryValidationError)} error={queryValidationError}>
            <MetricsPicker
          ...
          <Field label="Dimensions">
            <DimensionsPicker
              ...
              selectedMetrics={current.metrics}
            />
          </Field>
