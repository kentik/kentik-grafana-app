...
1.### Plugin type and ID changed

v1.7.0 was an `app` plugin with ID `kentik-connect-app`.  
v2.0.0 is a `datasource` plugin with ID `kentik-connect-datasource`.

Grafana enforces that datasource plugins must use the `-datasource` ID suffix, so v2.0.0 is published as a new catalog entry. **This is a feature, not a limitation** — both plugins can run simultaneously on the same Grafana instance, so you can migrate dashboards incrementally without any disruption to existing users.

`kentik-connect-app` v1.7.0 remains fully functional alongside the new plugin. There is no urgency to migrate.

**To adopt v2.0.0:**

1. Install `kentik-connect-datasource` from the Grafana plugin catalog (alongside the existing plugin — no need to remove it first)
2. Add a new datasource instance and enter your Kentik email and API token
3. Migrate dashboards to the new datasource at your own pace — old dashboards on `kentik-connect-app` continue to work unchangedmmary](#whats-new--summary)
